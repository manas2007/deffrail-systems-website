...
import styles from "./contactModalStyles.module.css";
...
import { EMAIL_REGEX, ERROR_THEME, FACEBOOK_LINK, INSTAGRAM_LINK, LINKEDIN_LINK, PHONE_REGEX, SERVICES, THEME, X_LINK, YOUTUBE_LINK } from '../constants/global';
...
const LetsTalkModal = (props) => {
    ...
    const [userInfo, setUserInfo] = useState({ name: "", email: "", number: "", message: "", profile: "", budget: "" });
    const [errors, setErrors] = useState({ name: "", email: "", number: "", message: "", profile: "", budget: "" });
    ...
    useEffect(() => {
        ...
        console.log("isFormSubmitted from localStorage:", isFormSubmitted);
        if (isFormSubmitted) {
        ...
        if (showRealTimeValidation) {
            ...
            switch (name) {
                ...
                case "number":
                    ...
                    break;
                case "profile":
                    newErrors.profile = value ? "" : "Profile selection is required";
                    break;
                case "budget":
                    newErrors.budget = value ? "" : "Budget selection is required";
                    break;
                default:
                    break;
            }
    ...
    const validateUserInfo = () => {
        ...
        if(!userInfo.profile) {
            errors.profile = "Profile selection is required";
        }
        if(!userInfo.budget) {
            errors.budget = "Budget selection is required";
        }

        console.log("ValidateUserInfo ::Errors :: ",errors);
    ...
    const submitData = async () => {
        try {
            ...
            if (response.data.leadDetails.insertedId) {
                return true
            } else {
                return false
            }
    ...
    const handleFormSubmission = async () => {
        ...
        console.log("HandleFormSubmission :: Success ::",success);
    ...
    return (
        <div className="relative">
            <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
                <div className={`my-6 mx-auto max-w-5xl bg-white opacity-[0.98] h-fit rounded-[10px] min-h-[] relative ${isMobile ? 'w-[95%] h-fit' : 'w-[30rem]'}`}>
                    {isFormSubmitted ? (
                        <div className={`${styles.formSuccessWrapper}`}>
                            <div className={`${styles.formSuccessUpperSection}`}>
                                <div className={`${styles.formSuccessUpperContent}`}>
                                    ...
                                    <div className="flex justify-between items-center pt-[1rem] w-[40%] mx-auto">
                                        <a href={INSTAGRAM_LINK}><InstagramIcon sx={{ color: "#ff00b1" }} /></a>
                                        <a href={YOUTUBE_LINK}><YouTubeIcon sx={{ color: "red", margin: "0 0.5rem" }} /></a>
                            ...
                            <div className={`${styles.letsTalkForm}`}>
                                ...
                                <div className={styles.textFieldWrapper}>
                                    <textarea
                                        ...
                                        onChange={handleChange}
                                        className="border-[2px] border-[#E2ECF5] w-full mt-[0.1rem] rounded-[10px] text-[0.85rem] px-[0.8rem] py-[0.5rem] outline-none"
                                    />
                                </div>
                                {/* <div className={`${styles.formFooter}`}>
                                    <button className='cursor-pointer' onClick={onSubmitClick}>{isLoading ? <Loader color="white" size="12px" /> : "Get Started"}</button>
                                </div> */}
                                <div className={`${styles.formFooter}`}>
                                    <DButton cta={onSubmitClick} text="Submit" theme="SOLID" />
                                </div>
                            </div>
                        </div>
                    )}
